...
function gui_fungal_shift()
	...
	if cooldown > 0 then
		...
		GuiText(gui, 0, 0, GameTextGetTranslatedOrNot("$lamas_stats_fungal_cooldown") .. " " .. cooldown)
		GuiImgId = GuiImgId + 1
	end
	GuiLayoutEnd(gui)
...
function gui_fungal_shift_get_seed_shifts(iter, convert_tries) --calculate shifts based on seed (code is copied from game itself)
    ...
    if not converted_any then
        GamePrint(GameTextGetTranslatedOrNot("$lamas_stats_fungal_predict_error") .. " " .. tostring(iter))
    end
...
function gui_fungal_shift_display_from(material)
	...
	if material.flask == "from" then --if flask was flagged
		if current_shifts < material.number then --if it's future shift
			...
			tooltiptext = tooltiptext .. GameTextGetTranslatedOrNot("$lamas_stats_fungal_shift_used") .. "\n"
		end
	end
	
	if ModSettingGet("lamas_stats.fungal_group_type") == "group" then
		if #material.from > 1 then
			GuiText(gui, 0, 0, GameTextGetTranslatedOrNot("$lamas_stats_fungal_group_of"), fungal_shift_scale)
		else
	...
	for i,mat in ipairs(material.from) do
		...
		tooltiptext = tooltiptext .. ", " .. GameTextGetTranslatedOrNot("$lamas_stats_ingame_name") .. (": ") .. mat .. "\n"
	end
	GuiEndAutoBoxNinePiece(gui,0,0,0,0,0,empty_png,empty_png)
...
function gui_fungal_shift_display_to(material)	
	...
	if material.flask == "to" then
		if current_shifts < material.number then --if it's future shift
			...
			if ModSettingGet("lamas_stats.enable_fungal_greedy_tip") then
				tooltiptext = tooltiptext .. GameTextGetTranslatedOrNot("$lamas_stats_fungal_greedy") .. "\n"
			end
	...
	if show_to then
		...
		tooltiptext = tooltiptext .. ", " .. GameTextGetTranslatedOrNot("$lamas_stats_ingame_name") .. (": ") .. material.to .. "\n"
	end
...
function gui_fungal_shift_display_past_shifts()
	for _,past_shift in ipairs(past_shifts) do
		...
		GuiText(gui, 0, 0, GameTextGetTranslatedOrNot("$lamas_stats_shift") .. " " .. tostring(past_shift.number) .. ": ", fungal_shift_scale)
...
function gui_fungal_shift_display_future_shifts()
	local nextshifttext = GameTextGetTranslatedOrNot("$lamas_stats_fungal_next_shift")
